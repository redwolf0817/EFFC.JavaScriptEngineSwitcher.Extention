...
namespace EFFC.ChakraCore.JsRt
{
    ...
    public struct JsValue
	{
        ...
        /// <summary>
		/// Gets a length of a <c>Array</c> value
		/// </summary>
		/// <remarks>
		/// Requires an active script context.
		...
		public int ArrayLength
        {
            get
            {
                if (this.ValueType == JsValueType.Array)
                {
                    int length;
                    var lengthid = new JsPropertyId();
                    var ret = NativeMethods.JsGetPropertyIdFromName("length", out lengthid);
                    length = this.GetProperty(lengthid).ToInt32();
